...
// Initialize the renderer and store a reference of a canvas_store
canvas_renderer::canvas_renderer(canvas_store& canvas): canvas(canvas)
{
	reset_view();
}
...
void canvas_renderer::screen_to_grid(int sx, int sy, int &gx, int &gy) const
{
	...
	gx = (int)floor((sx-trans_x)/cell_size);
	gy = (int)floor((sy-trans_y)/cell_size);
}
...
void canvas_renderer::snap_screen_coords(int sx, int sy, int &scx, int &scy) const
{
	...
	scx = (int)((floor((sx-trans_x)/cell_size) + 0.5) * cell_size + trans_x);
	scy = (int)((floor((sy-trans_y)/cell_size) + 0.5) * cell_size + trans_y); 
}
...
{
	double cell_x = (glutGet(GLUT_WINDOW_WIDTH)-20)*zoom_factor/(double)canvas.get_width();
	double cell_y = (glutGet(GLUT_WINDOW_HEIGHT)-40)*zoom_factor/(double)canvas.get_height();
